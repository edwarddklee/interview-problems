# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 13:03:31 2019
...
def find_geo_seq(l, r):
    ...
    for number in l:
        pos_first = x #i 
        var_second = number * r #L[j]
        var_third = var_second * r #L[k]
        for i in [i for i,x in enumerate(l) if x == var_second]:
            pos_second = i #j
            if pos_second > pos_first: #makes it so it doesn't wrap
                for j in [j for j,y in enumerate(l) if y == var_third]:
                    if j: #
                        pos_third = j
                        if pos_third > pos_second: #same concept
                            #append onto mega list
                            l_final.append(pos_first) 
                            l_final.append(pos_second)
                            l_final.append(pos_third)  
        x += 1 #next on the list


    
    t_final = tuple(l_final) 
    #splits tuple into 3's
    t_split = tuple(t_final[n:n + 3] for n, i in enumerate(t_final) if n % 3 == 0)
    count = len(t_split)
    #returns count of triplets 
    return count

def construct_test_cases(unsorted=True):
    """
    ...
    #test case that gets MemError, from initial sorted cases, everything else seems to work
    test_cases += [([345]*10000, 1, 166616670000)]
